...
Global Const AppRegKey = "HMC-FTP"
...
Function Scramble(ByVal text As String) As String
  
  Dim j As Long
 ' Debug.Print
  For j = 1 To Len(text)
   ' Debug.Print Asc(MID(text, j, 1))
    Mid(text, j, 1) = Chr(255 - Asc(MID(text, j, 1)))
...
End Function
Function UnScramble(ByVal text As String) As String
  Dim j As Long
  'Debug.Print
  For j = 1 To Len(text)
    Mid(text, j, 1) = Chr(255 - Asc(MID(text, j, 1)))
